...
for ($i = 0; $i -lt 10; $i++) {
    try {
        ...
        if ($eval.application_id) {
            ...
            Write-Host "  ✓ Pred $($i+1) | App: $($eval.application_id) | $($eval.tahmin) | Model: $model" -ForegroundColor Green
        }
    } catch {
        Write-Host "  ✗ Pred $($i+1) FAILED: $($_.Exception.Message)" -ForegroundColor Red
    }
}
...
try {
    ...
    if ($metrics -match 'finwise_model_route_total') {
        Write-Host "  ✓ Prometheus metrics present" -ForegroundColor Green
        $passed++
    } else {
        Write-Host "  ✗ No routing metrics" -ForegroundColor Red
        $failed++
    }
} catch {
    Write-Host "  ✗ FAILED: $($_.Exception.Message)" -ForegroundColor Red
...
Write-Host "  Success Rate: $(($passed / 13 * 100).ToString('F1'))%"
Write-Host ""
...
if ($passed -eq 13) {
    ...
    Write-Host "✓ 10 applications created"
    Write-Host "✓ Metrics endpoint operational"
    Write-Host "✓ Ready for GATE 2 (Model Routing Logic)"
    exit 0
} else {
    Write-Host "❌ GATE 1: FAILED ($failed tests)" -ForegroundColor Red
    Write-Host "════════════════════════════════════════════════════════════" -ForegroundColor Cyan
    exit 1
}
